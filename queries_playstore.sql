...
--SELECT app_name, MAX(reviews) FROM analytics WHERE rating >= '4.8' GROUP BY reviews;
...
SELECT category, COUNT(*)t FROM analytics GROUP BY category HAVING COUNT(*) > 300;
...
SELECT app_name, reviews, min_installs, min_installs/reviews as proportion FROM analytics WHERE min_installs > 1000 ORDER by proportion asc LIMIT 1;
